...
const ProjectDetailPage = () => {
  ...
  const renderDetailRow = (label: string, value: string | number | null | undefined) => {
    ...
    return (
      <TableRow>
        <TableCell className="font-medium text-gray-900 dark:text-white">{label}</TableCell>
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <Button onClick={() => router.push('/industry')} className="mb-6">
        &larr; Back to Search
      </Button>
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <Card>
            ...
            <Table striped>
              <TableBody className="divide-y">
                ...
                {renderDetailRow("Rank Category", project.project_rank_category)}
                {renderDetailRow("Quartile Bucket", project.quartile_bucket)}
                {renderDetailRow("Weighted Score Index", project.weighted_score_index?.toFixed(4))}
                {renderDetailRow("Weighted Score SMA", project.weighted_score_sma?.toFixed(4))}
              </TableBody>
            </Table>
          </Card>
          <Card>
            <h2 className="text-xl font-semibold mb-3">Activity Metrics</h2>
             <Table striped>
              <TableBody className="divide-y">
                ...
                {renderDetailRow("Original Ratio (Not Fork)", project.is_not_fork_ratio?.toFixed(4))}
              </TableBody>
            </Table>
          </Card>
        </div>
         <Card>
            ...
            <Table striped>
                ...
                <TableBody className="divide-y">
                    {renderDetailRow("Contributors % Change", formatPercentage(project.contributor_count_pct_change_over_4_weeks))}
                    {renderDetailRow("Forks % Change", formatPercentage(project.fork_count_pct_change_over_4_weeks))}
                    {renderDetailRow("Stars % Change", formatPercentage(project.stargaze_count_pct_change_over_4_weeks))}
                    {renderDetailRow("Commits % Change", formatPercentage(project.commit_count_pct_change_over_4_weeks))}
                    {renderDetailRow("Watchers % Change", formatPercentage(project.watcher_count_pct_change_over_4_weeks))}
                    {renderDetailRow("Original Ratio % Change", formatPercentage(project.is_not_fork_ratio_pct_change_over_4_weeks))}
                </TableBody>
            </Table>
        </Card>
        {/* You can add more cards or sections for other details like rank changes over 4 weeks etc. */}
      </Card>
    </div>
  );
};
